...
function validateJson(data) {
  ...
  try {
    ...
    // check id field (only numbers)
    if (!obj.id || obj.id.length == 0 || obj.id.length > 20 || !(/^\d+$/.test(obj.id))) {
      response.reason = '[!] Please check the id';
      return response;
    }
    // check data field (only numbers, letters and a white list of good characters)
    if (!obj.data || obj.data.length == 0 || obj.data.length > 200 || !(/^[a-z\d _:,.]*$/i.test(obj.data))) {
      response.reason = '[!] Please check the data';
      return response;
    }
    // check parent id field (only numbers)
